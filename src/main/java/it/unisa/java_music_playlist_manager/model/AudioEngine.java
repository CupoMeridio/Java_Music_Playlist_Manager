...
/**
 * Porta (Port in stile Ports &amp; Adapters (Hexagonal Architecture) per
 * l'audio.
 * <p>
 * Il dominio ({@code model} layer dipende da questa interfaccia — MAI da una
 * libreria concreta
 * (es. JavaFX MediaPlayer).
 * L'implementazione concreta (es. {@code JavaFXAudioEngine}) vive nel layer
 * {@code ui}.
 *
 * <h3>Convenzioni tempo</h3>
 * Tutti i valori temporali (seek, currentTime, totalDuration) sono in
 * <b>secondi</b>
 * come {@code double} &mdash; cos&igrave; l'interfaccia &egrave;
 * framework-agnostica.
 *
 * <h3>Listeners</h3>
 * I listener sono callback semplici ({@link Runnable}, {@link Consumer},
 * {@link DoubleConsumer})
 * invece di JavaFX {@code Observable} &mdash; per mantenere l'interfaccia
 * disaccoppiata.
 */
public interface AudioEngine {

    /**
     * Carica un file audio e (opzionalmente) registra i listener per gli eventi di
     * lifecycle.
